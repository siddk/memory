...
def train(train_story, train_questions, train_qstory, memory, model, loss, general_config):
    """
    Run Memory Network Training (no Linear Start)
    ...
    for ep in range(nepochs):
        ...
        for _ in Progress(range(int(math.floor(train_len / batch_size)))):
            ...
            for b in range(batch_size):
                # NOTE: +1 since train_questions[1, :] is the index of the sentence right before
                # the training question.
                # d is a batch of [word indices in sentence, sentence indices from batch] for
                # this story
                ...
                if enable_time:
                    ...
                    if randomize_time > 0:
                        # Random number of blank (must be < total sentences until the training
                        # question?)
                        ...
                        # Add random time (must be > dictionary's length) into the time word (
                        # decreasing order)
                        memory[0].data[-1, :d.shape[1], b] = np.sort(rt[:d.shape[1]])[::-1] + len(
                            dictionary)
...
def train_linear_start(train_story, train_questions, train_qstory, memory, model, loss,
                       general_config):
    """
